...
from dfa.duration_extraction import (
...
class Extractor:
    ...
    def extract_durations_for_item(
        self, item_file: Tuple[dict, Path, Path]
    ) -> Tuple[dict, np.array]:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Duration extraction for DeepForcedAligner."
    )
    parser.add_argument(
        "--config",
        "-c",
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Points to the a model file to restore.",
    )
    parser.add_argument(
        "--target", "-t", default="output", type=str, help="Target path"
    )
    parser.add_argument(
        "--batch_size", "-b", default=8, type=int, help="Batch size for inference."
    )
    parser.add_argument(
        "--num_workers",
        "-w",
    ...
    dur_target_dir, pred_target_dir = (
        Path(args.target) / "durations",
        Path(args.target) / "predictions",
    )
    ...
    ), "Symbols from dataset do not match symbols from model checkpoint!"
    ...
    print("Performing STT model inference...")
